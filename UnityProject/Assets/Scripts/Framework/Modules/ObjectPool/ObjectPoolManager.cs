...
namespace Framework.Modules.ObjectPool
{
    ...
    internal sealed partial class ObjectPoolManager : IObjectPoolManager
    {
        ...
        private readonly Dictionary<TypeNamePair, ObjectPoolBase> objectPools;
        ...
        public ObjectPoolManager()
        {
            ...
            MonoManager.Instance.AddListener(Update);
        }

        /// <summary>
        /// 获取对象池数量。
        /// </summary>
        public int Count => objectPools.Count;

        private void Update()
        {
            foreach (KeyValuePair<TypeNamePair, ObjectPoolBase> objectPool in objectPools)
            {
                objectPool.Value.Update(UnityEngine.Time.deltaTime, UnityEngine.Time.unscaledDeltaTime);
            }
        }
        ...
        {
            MonoManager.Instance.RemoveListener(Update);
        ...
        {
            return objectPools.ContainsKey(typeNamePair);
        }
        ...
        {
            if (objectPools.TryGetValue(typeNamePair, out var objectPool))
            {
                objectPool.Shutdown();
