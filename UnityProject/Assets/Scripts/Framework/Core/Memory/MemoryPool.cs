...
namespace Framework.Core.Memory
{
    ...
    public static partial class MemoryPool
    {
        ...
        /// 获取或设置是否开启强制检查。
        /// </summary>
        public static bool EnableStrictCheck { get; set; } = AppConst.DevMode;

        /// <summary>
        /// 获取内存池的数量。
        /// </summary>
        // ReSharper disable once InconsistentlySynchronizedField
        public static int Count => memoryCollections.Count;
        ...
        {
            int index = 0;
            MemoryPoolInfo[] results;

            lock (memoryCollections)
            {
                results = new MemoryPoolInfo[memoryCollections.Count];
                foreach (KeyValuePair<Type, MemoryCollection> memoryCollection in memoryCollections)
                {
                    results[index++] = new MemoryPoolInfo(memoryCollection.Key, memoryCollection.Value.UnusedMemoryCount, memoryCollection.Value.UsingMemoryCount,
                        memoryCollection.Value.AcquireMemoryCount, memoryCollection.Value.ReleaseMemoryCount, memoryCollection.Value.AddMemoryCount,
                        memoryCollection.Value.RemoveMemoryCount);
                }
            }
        ...
        private static MemoryCollection GetMemoryCollection(Type memoryType)
        {
            ...
            lock (memoryCollections)
            {
                if (!memoryCollections.TryGetValue(memoryType, out memoryCollection))
                {
                    memoryCollection = new MemoryCollection(memoryType);
                    memoryCollections.Add(memoryType, memoryCollection);
                }
            }
